...
def snapshot(slug: str, run: Path) -> dict:
    ...
    book = [dict(kind=b["kind"], protocol=b["protocol"], venue=b["venue"], symbol=b.get("symbol"),
                 ...
                 apy=b.get("apy"), venue_tvl_usd=b.get("venue_tvl_usd"), untracked=b.get("untracked", False),
                 balance=b.get("balance")) for b in a["book"] if fnum(b["usd"]) >= 1]
    ...
    return dict(
        ...
        book=book, permitted=permitted, ops_tools=ops,
